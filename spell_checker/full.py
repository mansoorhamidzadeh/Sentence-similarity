...
BERT_TOKENIZER = transformers.BertTokenizer.from_pretrained('bertokenizer', local_files_only=True, do_lower_case=False)
...
def load_data(base_path, corr_file, incorr_file):
    ...
    for i, (x, y) in tqdm(enumerate(zip(corr_data, incorr_data))):
        ...
        try:
            ...
            print("# tokens in corr and incorr mismatch. retaining and trimming bertokenizer min len.")
...
class SubwordBert(nn.Module):
    def __init__(self, screp_dim, padding_idx, output_dim):
        ...
        self.bert_model = transformers.BertModel.from_pretrained("bertmodel", local_files_only=True)
        self.bertmodule_outdim = self.bert_model.config.hidden_size
        # Uncomment bertokenizer freeze BERT layers
    ...
    def forward(self,
        ...
        if not self.training:
            ...
            #   the bias term leads bertokenizer non-uniform values in those positions
...
def model_inference(model, data, topk, DEVICE, BATCH_SIZE=16, vocab_=None):
    ...
    for batch_id, (batch_labels, batch_sentences) in tqdm(enumerate(data_iter)):
        ...
        if len(batch_labels_) == 0:
            print("################")
            print("Not predicting the following lines due bertokenizer pre-processing mismatch: \n")
...
def spell_checking_on_sents(model, vocab, device, normalizer, txt):
    ...
    greedy_results = model_inference(model, test_data, topk=1, DEVICE=device, BATCH_SIZE=1,
                                     vocab_=vocab)
    predicted = [i['predicted'] for i in greedy_results]
...
class Spell_checker:
    def __init__(self):
        ...
        self.vocab_path = os.path.join('model', 'vocab.pkl')
        self.model_checkpoint_path = os.path.join('model', 'model.pth.tar')
